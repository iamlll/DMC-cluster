...
def PlotReblockedE_vs_kcut(df):
    ...
    for tau,grp in df.groupby("tau"): 
        print(tau)
        Es = grp['eavg'].values
        E_err = grp['err'].values
        Ncuts = grp['Ncut'].values
        print(Ncuts)
        Ls = np.full(len(Ncuts),(2*4*np.pi/3)**(1/3) * rs) #sys size/length measured in a0; multiply by 2 since 2 = # of electrons
        idxs = np.argsort(Ncuts)
        Ls = Ls[idxs]
        kcuts = 2*np.pi*Ncuts[idxs]/Ls
        Es = Es[idxs]
        E_err = E_err[idxs]
        ax.plot(kcuts,Es,'.-',label='data,$\\tau=%.2f$' %tau)
        ...
        slope = -2/(np.pi)*(1-eta) #analytical semiclassical result predicts prefactor of 2, not 3/2
    #slope = 0.1
        pred = slope*kcuts + eta/dist #semiclassical prediction E_bi(r) = -(1-eta)*2*k_cut/pi + eta/r
        pred_llp = -alpha/(2*l**2)*2
        pred_str = -2*alpha**2/(2*l**2*3*np.pi)
        #print(pred_str)
        #print(Es)
        #ax.plot(kcuts,pred,'k.-',label='semiclassical')
    #ax.axhline(pred_str,c='red',label='$-2\\alpha^2 \hbar \omega/(3\pi)$')
